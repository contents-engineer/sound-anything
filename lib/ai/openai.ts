...
export class OpenAIProvider {
  ...
  async generate(opts: Selections, mode: GenerationMode, extras?: GenerationExtras): Promise<Omit<GenerationResult, 'generatedAt' | 'provider'>> {
    ...
    try {
      ...
      parsed = await call('이전 응답이 JSON 스키마를 어겼습니다. 반드시 {"prompt": string, "songs": null | Array<{title,titles,concept,stylePrompt,excludeStyles,recommendedModel,sliderHint,trackRole,lyrics}>} 형태로만 답하세요.')
    }
